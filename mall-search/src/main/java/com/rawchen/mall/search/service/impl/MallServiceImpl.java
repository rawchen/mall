package com.rawchen.mall.search.service.impl;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.TypeReference;
import com.rawchen.common.to.es.SkuEsModel;
import com.rawchen.common.utils.R;
import com.rawchen.mall.search.config.MallElasticSearchConfig;
import com.rawchen.mall.search.constant.EsConstant;
import com.rawchen.mall.search.feign.ProductFeignService;
import com.rawchen.mall.search.service.MallService;
import com.rawchen.mall.search.vo.AttrResponseVo;
import com.rawchen.mall.search.vo.BrandVo;
...
//		System.out.println("esModelsSize: "+esModels.size());
		List<Long> temp = new ArrayList<>();
		List<SkuEsModel> esNewModels = new ArrayList<>();
		for (SkuEsModel esModel : esModels) {
//			System.out.println(esModel.getSpuId());
			Long spuId = esModel.getSpuId();
			if (!temp.contains(spuId)) {
				temp.add(spuId);
				esNewModels.add(esModel);
			}
		}
//		System.out.println("size: " + esNewModels.size());
		result.setProducts(esNewModels);
...
//		long total = hits.getTotalHits().value;
		long total = result.getProducts().size();

		result.setTotal(total);

		// 总页码：计算得到
		int totalPages = (int)(total / EsConstant.PRODUCT_PASIZE + 1);
		...
		if(Param.getBrandId() != null && Param.getBrandId().size() > 0){
			...
			if(r.getCode() == 0){
				...
				for (BrandVo brandVo : brand) {
					System.out.println("brand: " + brandVo);
				}
